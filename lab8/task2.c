...
void PrintSymbols(){
    ...
    for(i=0 ; i<header->e_shnum ; ++i){
        ...
        if (section_header[i].sh_type == SHT_SYMTAB || section_header[i].sh_type == SHT_DYNSYM){
            ...
            sym_table = map_start + section_header[i].sh_offset;
            current_sym_table_size = (section_header[i].sh_size / section_header[i].sh_entsize);
            section_name = (void *) map_start + shstrnTable->sh_offset + section_header[i].sh_name;
            
            if (section_header[i].sh_type == SHT_SYMTAB)
                printf("Symbol table '.symtab' contains %d entries:\n",current_sym_table_size);
            else printf("Symbol table '.dynsym' contains %d entries:\n",current_sym_table_size);
                
            printf("  %s %-16s %-16s %-16s %s\n","[Nr]","Value","Section_Index","Section_Name","Symbol_Name");
            
            for (j=0 ; j< current_sym_table_size ; ++j){
                sym_name = (char *) (map_start + section_header[section_header[i].sh_link].sh_offset + sym_table[j].st_name);

                printf("  [%2d]",j);
                printf(" %016lx",sym_table[j].st_value);
                
                if (sym_table[j].st_shndx == 0)
                    printf(" %-16s","UND");
                else if (sym_table[j].st_shndx == 65521)
                    printf(" %-16s","ABS");
                else printf(" %-16d",sym_table[j].st_shndx);
